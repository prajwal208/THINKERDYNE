...
const TESTIMONIALS = [
  ...
  },
  {
    ...
    text: 'Thinkerdyne is really well designed, built and supported. They give a lot of value for the money. I appreciate how convenient it is to use a single partner for all our PCB and electronics engineering needs across every project.',
    rating: 5,
    initial: 'C',
    platform: 'G',
  },
...
function TestimonialCard({ t, isExpanded, onToggle }) {
  ...
  const displayText = isExpanded || !isLong ? t.text : t.text.slice(0, truncateLength) + '...';

  return (
    <div className="testimonials__card">
      {t.platform && (
        <span className="testimonials__card-platform" aria-hidden="true">{t.platform}</span>
      )}
    ...
    <section id="testimonials" className="testimonials section section--alt">
      <div className="container">
        <div className="testimonials__header" ref={headerRef}>
          ...
          <p className="testimonials__subtitle">What our clients say about us</p>
        </div>
        <div className="testimonials__grid" ref={gridRef}>
          {TESTIMONIALS.map((t) => (
            <TestimonialCard
              ...
              onToggle={() => setExpandedId((prev) => (prev === t.name + t.role ? null : t.name + t.role))}
            />
          ))}
        </div>
      </div>
